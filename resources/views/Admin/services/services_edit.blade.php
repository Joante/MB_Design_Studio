  ...
  <div class="bs-stepper-header">
    ...
    <div class="bs-stepper-content">
      <div id="account-details" class="content active dstepper-block">
        <form class="form" method="POST" action="{{ route('services_update', $service->id) }}" id="form" enctype="multipart/form-data">
            @csrf
            <div class="row">
                <div class="col-md-6 col-12">
                    <div class="form-group">
                        <label for="name-column">Nombre *</label>
                        <input type="text" id="name-column" class="form-control @error('title') is-invalid @enderror" placeholder="Nombre" name="title" required value="{{ old('title', $service->title) }}">
                        @error('title')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group">
                      @if (old('principal_page', $service->principal_page) == '1')
                        <input type="checkbox" class="custom-control-input" id="principal_page" name="principal_page" value=1 checked> 
                      @else
                ...
                <div class="col-md-6 col-12">
                    <div class="form-group">
                        ...
                        <textarea id="description-column" class="form-control @error('description') is-invalid @enderror" name="description" placeholder="Descripcion" required value="{{ old('description') }}">{{ old('description', $service->description) }}</textarea>
                        @error('description')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
          ...
          <div class="form-group col-md-12 d-flex justify-content-center">
            @foreach ($icons as $icon)
              <label style="margin-left: 30px;">
                @if (old('icon', $service->icon->id) == $icon->id)
